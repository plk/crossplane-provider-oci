...
func main() {
	var (
		app              = kingpin.New(filepath.Base(os.Args[0]), "OCI support for Crossplane - distributed Service.").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-oci-distributed"))
	if debugValue {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-oci-distributed",
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated_distributed(mgr, ClusterOpts), "Cannot setup distributed Template controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated_distributed(mgr, NamespacedOpts), "Cannot setup distributed Template controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup_distributed(mgr, ClusterOpts), "Cannot setup distributed Template controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup_distributed(mgr, NamespacedOpts), "Cannot setup distributed Template controllers")
	}
